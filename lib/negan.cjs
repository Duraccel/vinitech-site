    ...
    try {
      ...
      if (response.status === 429) {
        ...
        try {
          ...
          const code = typeof error?.type === 'string' && /^[a-z_]{1,60}$/.test(error.type) ? error.type : 'unknown';
          // Classify provider errors without retaining the error body or conversation.
          category = {type: code, freeTier: /free tier|free-tier/.test(detail), tokens: /token/.test(detail), requests: /request/.test(detail), quota: /quota|credit|budget/.test(detail)};
        } catch {}
        report({event: 'negan_rate_limited', status: 429, retryAfter: wait, ...category});
