...
function guided(messages) {
  const q = messages.at(-1).content.normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase();
  ...
  if (/como|funciona|extrator/.test(q)) return {...base, reply: 'Você pesquisa empresas por segmento e região, organiza as oportunidades e exporta informações disponíveis. Por exemplo, quem oferece TI pode avaliar escritórios de contabilidade na região que atende. É um exemplo de uso, não uma busca ao vivo. Quer comparar os planos?', actions: ['Comparar planos']};
...
function createHandler(env = process.env, fetcher = fetch) {
  return async (req, res) => {
    ...
    const fallback = () => res.status(200).json(guided(messages));
    ...
    try {
      ...
      const response = await fetcher('https://ai-gateway.vercel.sh/v1/chat/completions', {
        ...
        body: JSON.stringify({model: env.NEGAN_MODEL, messages: [{role: 'system', content: SYSTEM}, ...messages], max_tokens: 450, response_format: {type: 'json_object'}}),
        signal: AbortSignal.timeout(20000),
      });
      if (!response.ok) return fallback();
      const data = await response.json();
      const out = JSON.parse(data.choices?.[0]?.message?.content);
      if (typeof out.reply !== 'string' || !out.reply.trim() || out.reply.length > 1200) return fallback();
...
module.exports = {createHandler, guided, validateMessages, PLANS, INTRO};
